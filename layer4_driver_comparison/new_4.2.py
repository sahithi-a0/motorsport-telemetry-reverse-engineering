...
FILES = {
    'Driver 3': r'C:\Users\sahit\OneDrive\Documents\assignments_ARCVD\Driver 3 Race 1.csv',
    'Driver 4': r'C:\Users\sahit\OneDrive\Documents\assignments_ARCVD\Driver 4 Race 1.csv',
    'Driver 5': r'C:\Users\sahit\OneDrive\Documents\assignments_ARCVD\Driver 5  Race 1.csv',
}
BEST_LAPS = {
...
for drv, path in FILES.items():
    ...
    segtimes = parse_seg_times(path)
    valid_times = segtimes[2:-1]
    data[drv] = {
        'full': df, 'lap': bl,
        'segtimes': segtimes,
...
# ── P1 · Race Lap Time Progression ───────────────────────────────────────────
fig1 = plt.figure(figsize=(18, 7))
fig1.suptitle('Race — P1: Lap Time Progression  |  consistency over 20+ laps',
              color='white', fontsize=12)
fig1.canvas.manager.set_window_title('R P1 — Lap Time Progression')
...
for drv in data:
    ...
    lns = list(range(3, len(vt)+3))
    ax1.plot(lns, vt, color=COLORS[drv], linewidth=1.5,
...
ax1.set_title('Downward trend = improving tyres  flat = consistent pace  spikes = incidents',
              loc='left', fontsize=9)
...
for drv in data:
    vt = data[drv]['valid_times']
    clean = [t for t in vt if t < np.median(vt)*1.1]
    lines1 += [
        (f'{drv} best',    BEST_TIMES[drv],                  COLORS[drv]),
        (f'{drv} avg',     f'{np.mean(clean):.1f}s',          COLORS[drv]),
        (f'{drv} spread',  f'{max(vt)-min(vt):.1f}s',         COLORS[drv]),
        (f'{drv} laps',    f'{len(vt)}',                       COLORS[drv]),
    ]
stat_panel(sp1, lines1, 'Race Pace')
fig1.tight_layout(rect=[0, 0, 1, 0.94])

# ── P2 · Speed Trace ─────────────────────────────────────────────────────────
fig2 = plt.figure(figsize=(16, 7))
fig2.suptitle('Race — P2: Speed Trace Overlay  |  best race lap each driver',
              color='white', fontsize=12)
fig2.canvas.manager.set_window_title('R P2 — Speed Trace')
...
ax2.set_title('Race pace with fuel load — compare to qualifying for technique change',
              loc='left', fontsize=9)
...
fig3.suptitle('Race — P3: Lap Delta  |  below zero = left driver faster at that point',
              color='white', fontsize=12)
fig3.canvas.manager.set_window_title('R P3 — Lap Delta')
...
for row, (d1, d2) in enumerate(pairs):
    ...
    gap = data[d1]['lap_time'] - data[d2]['lap_time']
...
fig4.suptitle('Race — P4: Lap Time Heatmap  |  green=fast  red=slow  |  consistency visible across rows',
              color='white', fontsize=12)
fig4.canvas.manager.set_window_title('R P4 — Lap Heatmap')

gs4 = gridspec.GridSpec(1, 2, figure=fig4, width_ratios=[6, 1],
                         wspace=0.04, top=0.88, bottom=0.10,
...
cbar = plt.colorbar(im, ax=ax4, pad=0.02, fraction=0.02)
...
ax4.set_title('Each cell = one lap time  |  star = best lap  |  consistent green row = strong race driver',
              loc='left', fontsize=9)
