...
for drv, path in FILES.items():
    ...
    data[drv] = {
        ...
        'lap_time': lt, 'lap_len': bl['Dist'].iloc[-1],
    }
...
fig5.suptitle('Race — P5: Throttle & Brake Overlay  |  best race lap each driver',
              color='white', fontsize=12)
fig5.canvas.manager.set_window_title('R P5 — Throttle & Brake')
...
ax5a.set_title('Throttle — compare race vs qualifying technique', loc='left', fontsize=9)
...
ax5b.set_title('Brake — heavier braking in race = more tyre stress', loc='left', fontsize=9)
...
fig6.suptitle('Race — P6: Trail Braking Gate  |  throttle>5% AND brake>5%',
              color='white', fontsize=12)
fig6.canvas.manager.set_window_title('R P6 — Trail Braking')
...
fig7.suptitle('Race — P7: Throttle Rate dThrottle/dt  |  aggression in race conditions',
              color='white', fontsize=12)
fig7.canvas.manager.set_window_title('R P7 — Throttle Derivative')
...
fig8.suptitle('Race — P8: Steering Rate dSteering/dt  |  smoothness under race load',
              color='white', fontsize=12)
fig8.canvas.manager.set_window_title('R P8 — Steering Derivative')
...
for row, drv in enumerate(data):
    ...
    ax.legend(fontsize=7, framealpha=0.3, loc='upper right')
    ax.set_xlim(0, max_dist + 20)
    ax.set_title(f'{drv}  |  max R: {np.max(ds_):.0f}  max L: {np.min(ds_):.0f}  mean: {np.mean(np.abs(ds_)):.0f} deg/s',
                 loc='left', fontsize=9)
    if row == 2: ax.set_xlabel('Distance from lap start (m)')
...
fig9.suptitle('Race — P9: GG Diagram Overlay  |  grip usage under race conditions',
              color='white', fontsize=12)
fig9.canvas.manager.set_window_title('R P9 — GG Diagram')
...
ax9.set_title('Dashed circle = grip envelope  |  wider = more grip extracted in race',
              loc='left', fontsize=9)
